#!/usr/bin/env bash
# 로컬 GGUF 추론 스모크 테스트 (llama.cpp Metal)
# 모델: 기본값은 DeepSeek-Coder-V2-Lite-Instruct Q4_K_M (추후 경로/URL 교체 가능)
...
if [[ ! -f "$MODEL" ]]; then
  echo "❌ 모델 파일 없음: $MODEL"
  echo "   예: wget -c -O \"\$HOME/models/deepseek.gguf\" \\"
  echo "     \"https://huggingface.co/bartowski/DeepSeek-Coder-V2-Lite-Instruct-GGUF/resolve/main/DeepSeek-Coder-V2-Lite-Instruct-Q4_K_M.gguf\""
  exit 1
fi
...
다음 문장을 3줄로 요약하고 핵심 bullet point로 정리해줘:
...
if [[ $EXIT_CODE -ne 0 ]]; then
  ...
  run_once 32768 128 "AI를 간단히 설명해줘"
fi
