...
**Mac에서 제목 API 스모크 (`restart-all` / `run-gateway` 가정):**

```bash
curl -fsS "http://127.0.0.1:3001/api/health"

export SERVICE_API_KEY="${SERVICE_API_KEY:-test-service-key}"
curl -N -sS -X POST "http://127.0.0.1:3001/internal/v1/content/title-from-text/stream" \
  -H "Content-Type: application/json" \
  -H "X-Service-Key: $SERVICE_API_KEY" \
  -d '{"text":"로컬 스모크용 짧은 본문입니다.","language":"auto","style":"neutral","maxLength":80,"inputMode":"digest"}' \
| tail -n 5
# 마지막 `event: done` 의 JSON에 "title" 필드가 있으면 OK
```

- 대용량 본문은 `MAX_BODY_BYTES`(기본 256KiB)와 `text` 상한(100k자)을 참고. `run-gateway`에서 `node`를 못 찾으면 `PATH`에 Node가 있도록 하거나 `NODE_BIN`을 설정하세요([scripts/run-gateway.sh](scripts/run-gateway.sh)).
...
- `GEMMAE4_HF_REPO`: 이미지 분석용 멀티모달 HF repo (기본 `ggml-org/Qwen2-VL-2B-Instruct-GGUF`)
- `GEMMAE4_N_GPU_LAYERS`: 이미지 슬롯의 GPU 레이어 수 (기본 `0`, 메모리 압박이 있으면 CPU-only 유지 권장)
- `ROUTER_MMPROJ_OFFLOAD`: 멀티모달 projector GPU 오프로딩 여부 (`0`이면 끔, 기본 `0`)
...
기존 공개 SSE 채팅 API와 별도로, 서버-투-서버 호출용 컨텐츠 API를 제공합니다(제목은 SSE 스트림).

- `POST /internal/v1/content/title-from-text/stream` (SSE)
...
지금처럼 Qwen2-VL 2B 백킹을 쓰는 경우에도 `llama.cpp` 최신판이 필요합니다.
