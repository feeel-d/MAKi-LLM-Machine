...
## 3) 이미지 -> 요약 본문 (핵심 추출, 약 2~5줄)

`POST /internal/v1/content/body-from-image`

이미지에 보이는 텍스트·수치·항목 등 **사실만 읽어** 짧게 정리합니다. `body`는 여전히 단일 문자열이며, 줄바꿈·불릿으로 **약 2줄(`short`) ~ 약 3줄(`medium`, 생략 시 기본) ~ 최대 약 4~5줄(`long`)** 수준을 목표로 합니다.
